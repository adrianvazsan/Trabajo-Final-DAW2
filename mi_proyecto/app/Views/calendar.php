		...
		$(document).ready(function () {
			...
			$.ajax({
				...
				success: function (response) {
					console.log("Events loaded from the server:", response);

					// Transformamos los eventos para que coincidan con el formato de FullCalendar
					const eventosFormateados = response.map(evento => ({
						id: evento.pk_id_event,  // Convertimos pk_id_event a id
						title: evento.title,
						start: evento.start_date, // Convertimos start_date a start
						end: evento.end_date,     // Convertimos end_date a end
						description: evento.description_eng
					}));

					console.log("Transformed events:", eventosFormateados);
			...
			if (!title) return; // Si el usuario no ingresa un título, no hacer nada

			const description = prompt('Event Description:'); 
			if (!description) return; // Si el usuario no ingresa una descripción, no hacer nada
			...
			$.ajax({
				...
				data: {
					...
					description_eng: description // Enviamos la descripción ingresada
				},
				success: function(response) {
					...
					if (response.success) {
						...
						alert("Error adding event: " + (response.error || "Unknown error"));
					}
				},
				error: function(xhr) {
						...
						$.ajax({
							...
							success: function (response) {
								if (response.success) {
									...
									alert("Error deleting the event: " + response.error);
								}
							},
							error: function (xhr, status, error) {
								console.error("Error in the elimination:", error);
							}
						});
					}
				}
